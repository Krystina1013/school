<template>
  <div class="app-container">
      ...
      <split-pane split="vertical" @resize="resize" :min-percent="10" :default-percent="rowSpan">
        <template slot="paneL">
          <div class="leftContent">
            <data-tree :page="page" type="classBuild" :initFormHeader="initForm" :TreeData="TreeData" :hideHandle="false" @fetchData="fetchData" @handleBuildingId="handleBuildingId" @getHouseTreeData="getHouseTreeData" />
          </div>
        </template>
        <template slot="paneR">
          <div class="rightContent">
                ...
                <action-header ref="actionHeader" :initFormHeader="initForm" :filterForm="filterForm" :dialogCreate.sync="dialogCreate" :moreStatus="false" :houseStatus="true" :total="page.total" @fetchData="fetchData" @handleCutStatus="handleCutStatus">
                  ...
                  <div slot="houseNum">
                    <div class="word-filter">
                      <span class="filter-name">教室名称:</span>
                      <el-input clearable @keyup.enter.native="emitFetchData" class="input-filter" maxlength="30" v-model.trim="filterForm.serialNumber" placeholder="输入教室名称" size="small"></el-input>
                    </div>
                    <div class="word-filter">
                      <span class="filter-name">所属楼层:</span>
                      <el-input clearable @keyup.enter.native="emitFetchData" class="input-filter" maxlength="2" @input="numberInput" v-model.trim="filterForm.storey" placeholder="输入所属楼层" size="small"></el-input>
                    </div>
                  </div>
                </action-header>
              </el-col>
            ...
            <el-table v-show="!cutBtnType" :data="list_data" stripe border v-loading="showLoading" highlight-current-row @cell-mouse-enter="enterCellChange" @selection-change="handleSelectionChange" @cell-mouse-leave="leaveCellChange">
              ...
              <el-table-column type="index" align="center" label="序号" class="indexNum" width="80">
                <template slot-scope="scope">
                  <span>{{ scope.$index + 1 }}</span>
                </template>
              </el-table-column>

              <el-table-column align="center" prop="locationName" :show-overflow-tooltip="true" label="教室">
                <template slot-scope="scope">
    ...
    <!-- 新建房屋 -->
    <el-dialog class="newDialog" :close-on-click-modal="false" :title="isUpdate ? '编辑教室' : '添加教室'" :visible.sync="dialogCreate" width="500px" :before-close="handleClose">
      <el-form :model="Form" :rules="rules" ref="Forms" label-width="100px" label-position="right" style="margin-right: 40px">
        <el-form-item label="教室楼栋:" prop="buildingId">
        ...
        <el-form-item label="教室名称:" prop="serialNumber">
          <el-input clearable v-model="Form.serialNumber" maxlength="10" placeholder="输入教室名称"></el-input>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="small" style="margin-leftL:18px" :disabled="addStatus" type="primary" @click="addHouseConfirm">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 人入住员信息 -->
    <el-dialog width="722px" :close-on-click-modal="false" class="newDialog" title="入住人员" :visible.sync="dialogFormVisible">
      <el-row :gutter="15" class="img-dialog" v-if="imgs.length > 0">
        <el-col :span="6" class="img-col" v-for="(item, i) in imgs" :key="i">
          <div class="img-con">
            <img class="img" :src="item.face" alt="" />
            <p class="name-item">{{ item.name }} {{ item.studentNo }}</p>
          </div>
        </el-col>
...
export default class classroomManage extends Vue {
  ...
  rules: any = {
    ...
    buildingId: [
      { required: true, message: "请选择教室所属楼栋", trigger: "change" },
    ],
    storeyNum: [
      {
        required: true,
  ...
  //       getRoomPeople(this.houseId).then(res => {
  ...
  // 获取房屋结构
  getHouseTreeData() {
    getBuildingTreeData({ buildingType: 2 }).then((res: Object) => {
  ...
  addHouseConfirm = _throttle(() => {
    // const Form ={
    //   note: this.Form['note'],
    //   buildingId: this.FormbuildingId,
    //   serialNumber: this.Form.serialNumber,
    //   id: this.Form.id,
    //   beds: this.Form.beds,
    //   storeyNum: this.Form.storey,
    //   roomType: 1
    // };
    console.log(this.Form);
