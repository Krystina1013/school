<template>
  <div class="app-container">
                ...
                <el-table ref="table" :data="tableData" stripe border highlight-current-row @sort-change="sortChange" v-loading="tableLoading" @cell-click="tableDbEdit" @cell-mouse-enter="enterCellChange" @cell-mouse-leave="leaveCellChange">
                  ...
                  <el-table-column prop="bindAddress" label="绑定位置" show-overflow-tooltip align="center">
                    <template slot-scope="scope">
                      ...
                      <!-- <svg v-if="scope.row.inOut == 'out'" class="icon myFont" aria-hidden="true">
                        <use xlink:href="#icon-a-Group1266"></use>
                      </svg>
                      <svg v-if="scope.row.inOut == 'in'" class="icon myFont" aria-hidden="true">
                        <use xlink:href="#icon-a-Group1267"></use>
                      </svg> -->
                    </template>
                  </el-table-column>
    ...
    <el-dialog class="video-dialog" :title="'实况：' + currentPlayCamera.name" :visible.sync="dialogVisible" :before-close="handleClose">
      <Reflv data-name="single" v-show="reflvUrl" :style="{ width: '840px', height: '540px',margin: '0 auto' }" :name="'single'" :cameraId="currentPlayCamera.id" :url="reflvUrl" ref="reflv" :isPc="isPc" />
    </el-dialog>
    ...
    <el-dialog class="newDialog" width="600px" title="预警设置" :visible.sync="dialogWarnVisible" :before-close="handleCloseWarn">
      <div>
        ...
        <div class="flex">
          ...
          <!-- <div class="flex earlyWarn" style="margin-left: 120px">
            <p>预警图限制：</p>
            <div style="margin-left: 20px">
              <p>
                <el-radio v-model="warnLimit" label="none">无限制</el-radio>
              </p>
              <p style="margin-top: 20px">
                <el-radio v-model="warnLimit" label="goodface">清晰正脸</el-radio>
              </p>
            </div>
          </div> -->
        </div>
    ...
    <!-- 分组编辑 -->
    <el-dialog width="450px" title="分组编辑" :visible.sync="editDialog" :before-close="handleCloseEdit">
...
export default {
  ...
  methods: {
    ...
    fetchData(option) {
      ...
      // this.filterForm["activeIp"] = option.params.activeIp;
      // this.filterForm["liveStatus"] = option.params.status;
      this.getCameraList();
    },
