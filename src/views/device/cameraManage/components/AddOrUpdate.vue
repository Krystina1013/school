...
import { addCamera, updateCamera } from "@/api/police";
...
export default class extends Vue {
  ...
  gpsChange() {
    ...
    if (this.gps.lat != "" && this.gps.lng != "") {
      var new_point = new map.Point(this.gps.lng, this.gps.lat);
      var gc = new map.Geocoder(); //初始化，Geocoder类
  ...
  initMap() {
    let that = this;
    this.$nextTick(() => {
      map = new map.Map("allmap", { enableMapClick: false });
      if (this.title == "修改") {
        map.clearOverlays();
        if (!this.PointerList.length) {
          ...
          if (!("lng" in this.PointerList[0])) {
            ...
            var new_point: any = new map.Point(
              this.PointerList[0].lng,
              this.PointerList[0].lat
            );
            let marker = new map.Marker(new_point);
      ...
      this.localSearch = new map.LocalSearch(map, {
  ...
  getLocationName(lng, lat, type) {
    ...
    let location_name = {};
    //获取定位名称
    var new_point = new map.Point(lng, lat);
    map.clearOverlays();
    let marker = new map.Marker(new_point);
    map.addOverlay(marker);
    var gc = new map.Geocoder(); //初始化，Geocoder类
