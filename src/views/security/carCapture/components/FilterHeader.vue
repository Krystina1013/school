            ...
            <transition name="el-zoom-in-top">
              <div v-show="visibleFilter" class="filterDialog">
                <div class="word-filter">
                  ...
                  <el-select v-model="picFilterForm.vehicleType" placeholder="请选择">
                    <el-option v-for="(item,i) in vehicleTypeArr" :key="i" :label="item" :value="item">
                    </el-option>
                  </el-select>
                </div>
                <div class="word-filter">
                  <span class="filter-name">车牌类型:</span>
                  <el-select v-model="picFilterForm.licensePlateType" placeholder="请选择">
                    <el-option v-for="(item,i) in licensePlateTypeArr" :key="i" :label="item" :value="item">
                    </el-option>
                  </el-select>
                </div>
                <div class="word-filter">
                  <span class="filter-name">车牌号:</span>
                  <el-input v-model.trim="picFilterForm.plateNumber" maxlength="10" style="width:217px"></el-input>
                </div>
                <div class="word-filter">
                  <span class="filter-name">车辆颜色:</span>
                  <el-input v-model.trim="picFilterForm.vehicleColor" maxlength="10" style="width:217px"></el-input>
                </div>
                <div class="word-filter">
                ...
                <div class="word-filter" v-show="time == 4">
                  ...
                  <el-date-picker v-model="picFilterForm.startTime" @change="startChange" type="datetime" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期时间" :clearable="false" size="mini" class="input-filter" style="width: 252px">
                  </el-date-picker>
                </div>
                <div class="word-filter" v-show="time == 4">
                  <span class="filter-name">结束时间:</span>
                  <el-date-picker v-model="picFilterForm.endTime" @change="endChange" type="datetime" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期时间" :clearable="false" size="mini" class="input-filter" style="width: 252px">
                  </el-date-picker>
                </div>
                <div class="word-filter" v-if="activeName == 'cap'">
                ...
                <div class="word-filter" v-if="deviceAll == '2' && activeName == 'cap'">
...
import { carTypeList, carNumArr } from "@/api/carApi";
@Component({
...
export default class extends Vue {
  ...
  licensePlateTypeArr: Array<any> = [];
  vehicleTypeArr: Array<any> = [];
  @Watch("activeName")
  changeActive() {
    if (!this.search) {
      this.clearPicSearch();
    ...
    carTypeList().then((res) => {
      if (res.data.code === 200) {
        ...
        for (let key in res.data.data) {
          ...
          this.vehicleTypeArr.push(data[key]);
        }
        // this.vehicleTypeArr = res.data.data;
      }
    });
  }
  // 车牌类型arr
  getCarNumArr() {
    this.licensePlateTypeArr = [];
    carNumArr().then((res) => {
      if (res.data.code === 200) {
        let data = res.data.data;
        for (let key in res.data.data) {
          this.licensePlateTypeArr.push(data[key]);
        }
        // this.licensePlateTypeArr = res.data.data;
      }
    });
  }
  startChange() {
  ...
  sizeChange() {
    this.picFilterForm.pageSize = this.size;
  ...
  reset() {
    ...
    this.picFilterForm.plateNumber = '';
  }
  clearFilterFormfresh() {
    ...
    this.picFilterForm.vehicleType = "";
    this.picFilterForm.licensePlateType = "";
    this.picFilterForm.plateNumber = "";
    this.picFilterForm.vehicleColor = "";
    this.picFilterForm.score = "";
    ...
    this.dateRange = [];
    if (this.activeName == "cap") {
      this.picFilterForm.startTime = getTodayStartTime();
