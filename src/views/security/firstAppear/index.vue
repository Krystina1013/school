        ...
        <div class="rightContent" v-loading="showLoading">
          ...
          >
            <div v-for="(list, i) in list" :key="i">
              ...
              <div class="more" v-if="list.count>10"><el-button :disabled="list.noMore" size="mini" type="primary" @click="getMore(list,i)" :loading="list.load">更多</el-button></div>
            </div>
          </div>
...
export default class extends Vue {
  ...
  //   item.load=true;
  //   this.$forceUpdate();
  //   firstAppearDay(param).then(res=>{
  //     let data = res.data.data;
  //     item.page = item.page+=1;
  //     item.load=false;
  //     if(data.records.length){
  //       item.firstAppearVoList = [...item.firstAppearVoList,...data.records]
  //     }else{
  //       item.noMore = true;
  //       this.$forceUpdate();
  //     }
