<template>
  <div>
    <!-- 选择预警人员 -->
    <el-dialog class="newDialog noPadding" title="自定义" :visible="true" width="1166px" top='50px' :before-close="closeDialogPerson" :close-on-click-modal="false">
      <el-row>
        <el-col :span="19">
          <div class="header_div left">
            <el-tabs v-model="activeName" @tab-click="changeRadio">
              <el-tab-pane label="教师" name="1"></el-tab-pane>
              <el-tab-pane label="职工" name="2"></el-tab-pane>
              <el-tab-pane label="家长" name="3"></el-tab-pane>
            </el-tabs>
          </div>
          <div class="body_div left">

            <div v-show="activeName === '1'">
              <el-form ref="form" :model="filterForm2" :inline="true" @submit.native.prevent>
              ...
              <el-table ref="table2" height="498" v-loading='tab2Laoding' :data="tableTeachersData" border stripe :header-cell-style="{background:'#f5f5f5',color:'#606266', fontWeight: '500'}">
            ...
            <div v-show="activeName === '2'">
              <el-form ref="form" :model="filterForm3" :inline="true" @submit.native.prevent>
                ...
                <el-form-item style="float:right">
                  <el-button type="primary" size="mini" @click="search3">查询</el-button>
                </el-form-item>
              </el-form>
              <el-table ref="table3" height="498" v-loading='tab3Laoding' :data="tableWorkersData" border stripe :header-cell-style="{background:'#f5f5f5',color:'#606266', fontWeight: '500'}">
                <el-table-column type="index" width="60" label="序号" align="center"></el-table-column>
            ...
            <div v-show="activeName === '3'">
              <el-form ref="form" :model="filterForm1" :inline="true">
                <el-form-item label="学生姓名:">
                  <el-input maxlength="30" v-model.trim="filterForm1.studentName" size="small" placeholder="请输入学生姓名" class="input_search"></el-input>
                </el-form-item>
              ...
              <el-table ref="table1" height="498" :data="tableStudentsData" border stripe v-loading='tab1Laoding' :header-cell-style="{background:'#f5f5f5',color:'#606266', fontWeight: '500'}">
                <el-table-column type="index" width="60" label="序号" align="center"></el-table-column>
                <el-table-column show-overflow-tooltip prop="relativeName" label="家长姓名" align="center">
                  <template slot-scope='scope'>
                    <span>{{ scope.row.relativeName || '-'}}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="relativePhone" label="电话" align="center">
                  <template slot-scope='scope'>
                    <span>{{ scope.row.relativePhone || '-'}}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="name" label="学生姓名" align="center">
                  <template slot-scope='scope'>
                    <span>{{ scope.row.name || '-'}}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="className" label="班级" align="center">
                  <template slot-scope='scope'>
                    <span>{{scope.row.className || '-'}}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="studentNo" label="学号" align="center">
                  <template slot-scope='scope'>
...
export default class DialogChoosePerson extends Vue {
  ...
  tab1Laoding: boolean = false;
  tab2Laoding: boolean = false;
  tab3Laoding: boolean = false;
  ...
  async getTableData1() {
    let params = {
      ...
      studentName: this.filterForm1["studentName"],
      studentNo: this.filterForm1["studentNo"],
      className: this.filterForm1["className"],
      hasFace: true,
    };
    this.tab1Laoding = true;
    let { data } = await getParents(params);
    this.tab1Laoding = false;
    data = data.data;
    ...
    if (data.records.length > 0) {
      data.records.map((item: any, i) => {
        ...
        let obj = this["selectedRoom"].find(
          (item1: any) => item1.userid == item.relativeUserId
        );
        if (obj) {
  ...
  async getTableData2() {
    ...
    this.tab2Laoding = true;
    let { data } = await getTeacher(params);
    this.tab2Laoding = false;
    data = data.data;
  ...
  async getTableData3() {
    ...
    this.tab3Laoding = true;
    let { data } = await getWorker(params);
    this.tab3Laoding = false;
    data = data.data;
    ...
    if (data.records.length > 0) {
      data.records.map((item: any, i) => {
        ...
        data.records[i].overTime = "";
        let obj = this["selectedRoom"].find(
          (item1: any) => item1.userid == item.scenceUserId
        );
        if (obj) {
          data.records[i].selected = true;
        } else {
  ...
  created() {
    ...
    let type2: any = [];
    let type3: any = [];
    _.mapValues(this.selectedRoom, (item: any) => {
      if (item.type === 1) {
        type1.push(item);
      } else if (item.type === 2) {
        type2.push(item);
    ...
    if (datas) {
      if (datas.student_scenceUserVos) {
        ...
        type1.map((stu: any) => {
          this.selectedRoomAll.push({
            type: 1,
            relativeName: stu.relativeName,
            relativePhone: stu.relativePhone,
          });
        });
      }
      ...
      this.selectedRoomAll = [...this.selectedRoom];
      console.log("selectedRoomAll", this.selectedRoomAll);
    }
    this.getGroupTree();
    this.getTableData1();
    this.getTableData2();
  ...
  cancelRoom(row) {
    ...
    this.selectedRoomAll.splice(index, 1);
  }

  confirm() {
    debugger;
    const errorData: any[] = [];
    this.selectedRoom.forEach((v: any) => {
      if (v.type == 1) {
        // 家长
        v.userid = v.relativeUserId;
        if(v.relativePhone == '' || v.relativePhone == null){
          errorData.push(v);
        }
      } else {
        v.userid = v.scenceUserId;
      }
    });
    if(errorData.length > 0){
      this.$message.error("选择的家长中有人无号码信息!");
      return;
    }
