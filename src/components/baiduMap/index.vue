...
-->
<template>
  <baidu-map @click="getPoint" :center="{ lng, lat }" :zoom="zoom" :style="{'height':height}" :scroll-wheel-zoom="true" class="map" @ready="handler">
    <!--是否需要搜索功能-->
    <!-- <slot name="search"></slot> -->
    <!-- <bm-local-search :autoViewport="autoVisible" :panel="false" :keyword="keyword" :location="location" v-if="searchStatus"></bm-local-search> -->
    <!-- <slot name="point"></slot> -->
    <bm-marker v-show="markerStatus" v-for="(item, index) in positionList" :position="item.position" :key="'point-' + index" :dragging="false" :icon="{
    ...
    <bm-marker v-show="backStatus" v-for="(item, index) in list" :position="item.position" :key="index" :dragging="false" :icon="{ url: locImg, size: { width: 30, height: 30 } }">
      <!-- <bm-info-window
            :show="item.show"
...
export default class BaiDuMap extends Vue {
  ...
  @Prop({ default: true }) clickStatus: boolean;
  @Prop({
    default: () => {
      return [];
    },
