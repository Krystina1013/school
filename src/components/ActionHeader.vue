  ...
  <el-row type="flex" class="row-bg">
    <el-col :span="12">
      <div class="leftAction">
        ...
        <template class="createBtn">
          <slot name="faceImport"></slot>
        </template>
        <el-dropdown class="createBtn" v-if="moreStatus" size="small" @command="handleClick">
    ...
    <el-col :span="12" justify="end">
      <div class="rightAction">
        <div class="content" style="display:none;" v-if="rourePath === '/house/list'" :style="{
          ...
          <span v-if="cutBtn">
            <i style="font-size:16px;" class="iconfont icon-gengduo1" :style="{ color: floorControl ? '' : '#C0C4CC' }"></i>
            列表视图
          </span>
        </div>

        <div v-if="filterStatus" class="content" ref="filterCon">
          <span class="filter-style" :class="[hasProty ? 'activeFilter' : '']" @click.stop="(visibleFilter = !visibleFilter), (visible = false)">
            <i :style="{ color: hasProty ? '#409eff' : '' }" class="iconfont icon-guolv"></i>
            过滤
          </span>
        ...
        <!-- 日期查询 -->
        <div v-if="searchStatus" class="filterDialog">
          <slot name="searchNum"></slot>
          <el-button @click="emitFetchData" class="fliterBtn" type="primary" size="small">查询</el-button>
        </div>
        ...
        <div v-if="!searchStatus">
          ...
          <i v-if="pageStatus" @click.stop="(visible = !visible), (visibleFilter = false)" style="font-size:16px; margin-right: 20px;cursor: pointer;" class="iconfont icon-shezhi"></i>
        </div>

        <transition v-if="pageStatus" name="el-zoom-in-top">
          <div v-show="visible" class="setting">
            <span>每页显示:</span>
            <el-select @change="sizeChange" style="width:100px;margin-left:10px" v-model="size" placeholder="请选择" v-if='!scroll'>
              <el-option v-for="item in pageSize" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
            <el-select @change="sizeChanges" style="width:100px;margin-left:10px" v-model="size" placeholder="请选择" v-else>
              <el-option v-for="item in scrollPageSize" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </div>
        </transition>
      </div>
...
import qs from "qs";
import { exportList } from "@/api/user";
...
export default class ActionManage extends Vue {
  @Prop({ default: 0 }) private total: any; // 显示筛选多少条记录
  @Prop({ default: 1 }) btnStatus: Number; //  btnStatus: 0 表示没有创建导出按钮 1 创建按钮 2 导出按钮
  @Prop({ default: false }) police: Boolean; //是否是安防模块
  ...
  @Prop({ default: false }) searchStatus: boolean; // 是否需要显示过滤条件
  @Prop({ default: true }) moreStatus: boolean; // 是否显示更多菜单
  // @Prop({ default: true }) faceImport: boolean = false; // 是否显示人
  @Prop({ default: true }) pageStatus: boolean; //是否显示分页
  @Prop({ default: false }) scroll: boolean; //是否是滑动翻页
  ...
  @Prop()
  private initFormHeader: object; // 初始化的地址 方式
  ...
  private scrollPageSize: Array<Object> = [
    // 数据显示条数数组--安防滑动显示数量
    {
      label: "20",
      value: 20,
  ...
  mounted() {
    ...
    document.addEventListener("click", (e) => {
      if (
        ((that as any).$refs.filterCon &&
          (that as any).$refs.filterCon.contains(e["target"] as HTMLElement)) ||
  ...
  emitFetchData() {
    ...
    // if(this.filterForm["typeCard"]=='card'){
    //   let param={
    //     houseName:this.filterForm['bindName']?`${this.filterForm['bindName']}-${this.filterForm["houseName"]}`:this.filterForm["houseName"],
    //     cardNo:this.filterForm['cardNo'],
  ...
  clearFilterFormfresh() {
    ...
    let logDate =
      this.initFormHeader["url"].indexOf("admin/attendance-record") != -1 ||
      this.initFormHeader["url"].indexOf("checkin/allschool") != -1;
    this.hasProty = false;
    this.visibleFilter = false;
    this.page["page"] = 1;
    for (let key in this.filterForm) {
      if (
        ...
        this.filterForm[key] = "";
      } else if (typeof this.filterForm[key] === "string" && !door && !user) {
        this.filterForm[key] = "";
      } else if (user || door) {
    ...
    if (this.filterForm["date"] == "" && logDate) {
      const date = new Date();
      this.filterForm["date"] = format(date, "yyyy-MM-dd");
    }
    this["phoneNum"] = 0;
    this.initFormHeader["params"] = Object.assign(
      this.initFormHeader["params"],
  ...
  clearFilterForm() {
    ...
    let logDate =
      this.initFormHeader["url"].indexOf(
        "admin/attendance-record/student/resident"
      ) != -1 ||
      this.initFormHeader["url"].indexOf("admin/attendance-record/student") !=
        -1;
    ...
    // if (this.filterForm["operateDate"] == "") {
    //   const date = new Date();
