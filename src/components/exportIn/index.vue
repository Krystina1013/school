<template>
  <div>
    <el-dialog class="newDialog" title="批量导入" :visible.sync="visible" width="670px" :before-close="handleClose">
        <el-upload class="upload-demo"  :show-file-list="true" :on-error="errorUpload" :on-success="successUpload" :multiple="false" :headers="header" :file-list="fileList" :action="uploadUrl" accept=".xlsx,.xls" :on-change="changeFile" ref="upload" :auto-upload="false">
      ...
      <el-upload class="upload-demo color" style="display:inline-block;margin-right:5px;text-align:left" :show-file-list="false" :on-error="errorUpload" :on-success="successUpload" :multiple="false" :headers="header" :file-list="fileList" :action="uploadUrl" accept=".xlsx,.xls" :on-change="changeFile" ref="upload" :auto-upload="false">
        <i class="el-icon-plus"></i>
        添加文件
        <br />
        <span style="color:black">{{ fileName }}</span>
      </el-upload> -->
      <!-- <h4>目前仅支持*.xlsx;添加文件后，<span style="color:#ff0342">请到【导入记录】中查看导入结果</span>。</h4> -->
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="handleClose">取 消</el-button>
        <el-button size="small" style="margin-left:18px" type="primary" @click="confirmUpload">确 定</el-button>
      </span>
    </el-dialog>
